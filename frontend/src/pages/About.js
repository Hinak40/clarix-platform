  ...
  return (
    <div style={{ backgroundColor: bg }}>
      ...
      <section style={{ background: 'linear-gradient(135deg, #080c14 0%, #0f172a 50%, #1a1040 100%)', padding: isMobile ? '80px 20px 60px' : '120px 80px 80px', textAlign: 'center' }}>
        ...
        <h1 style={{ ...styles.heroTitle, fontSize: isMobile ? '36px' : '56px' }}>We Are Clarix</h1>
        <p style={{ ...styles.heroSubtitle, fontSize: isMobile ? '15px' : '18px' }}>A passionate team of developers, designers, and strategists building digital solutions that matter.</p>
      </section>
      ...
      <section style={{ padding: isMobile ? '60px 20px' : '80px 80px', backgroundColor: bg }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '25px' }}>
          {[
            ...
            <div key={i} style={{ backgroundColor: cardBg, border: `1px solid ${border}`, padding: isMobile ? '30px 20px' : '40px 35px', borderRadius: '16px' }}>
      ...
      <section style={{ padding: isMobile ? '60px 20px' : '80px 80px', backgroundColor: darkMode ? '#0a0f1e' : '#f1f5f9', textAlign: 'center' }}>
        <span style={{ ...styles.tag }}>WHAT DRIVES US</span>
        <h2 style={{ fontSize: isMobile ? '28px' : '34px', fontWeight: '800', color: titleColor, marginBottom: '40px' }}>Our Core Values</h2>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: '20px' }}>
          {values.map((v, i) => (
            <div key={i} style={{ backgroundColor: cardBg, border: `1px solid ${border}`, padding: '28px 20px', borderRadius: '14px', textAlign: 'center' }}>
      ...
      <section style={{ padding: isMobile ? '60px 20px' : '80px 80px', backgroundColor: bg }}>
        ...
        <h2 style={{ fontSize: isMobile ? '28px' : '34px', fontWeight: '800', color: titleColor, marginBottom: '10px' }}>Meet Our Team</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: '20px' }}>
          {team.map((member, i) => (
            <div key={i} style={{ backgroundColor: cardBg, border: `1px solid ${border}`, padding: '30px 20px', borderRadius: '16px', textAlign: 'center' }}>
              <div style={{ width: '65px', height: '65px', borderRadius: '50%', backgroundColor: member.color, color: 'white', fontSize: '26px', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 15px' }}>
                {member.avatar}
              </div>
      ...
      <section style={{ display: 'flex', backgroundColor: '#6366f1', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={i} style={{ flex: isMobile ? '1 1 50%' : 1, textAlign: 'center', padding: '28px 15px', borderRight: '1px solid rgba(255,255,255,0.15)' }}>
...
const styles = {
  ...
  heroTitle: { fontWeight: '800', color: 'white', marginBottom: '20px', letterSpacing: '-1.5px' },
  heroSubtitle: { color: '#64748b', lineHeight: '1.7' }
};
