...
import { getBlogs, createBlog, deleteBlog } from '../api/index';

function ManageBlog() {
  const [blogs, setBlogs] = useState([]);
  const [form, setForm] = useState({ title: '', content: '', author: 'Clarix Team' });

  const fetchBlogs = () => {
    getBlogs().then(res => setBlogs(res.data)).catch(() => {});
  };
  ...
  const handleAdd = async (e) => {
    ...
    try {
      ...
      setForm({ title: '', content: '', author: 'Clarix Team' });
      fetchBlogs();
    } catch {}
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.main}>
        <h1 style={styles.pageTitle}>Manage Blog</h1>
        <form onSubmit={handleAdd} style={styles.form}>
          <input style={styles.input} placeholder="Blog Title" value={form.title} onChange={e => setForm({...form, title: e.target.value})} required />
          <textarea style={styles.textarea} placeholder="Blog Content" rows="5" value={form.content} onChange={e => setForm({...form, content: e.target.value})} required />
          <input style={styles.input} placeholder="Author" value={form.author} onChange={e => setForm({...form, author: e.target.value})} />
          <button type="submit" style={styles.btn}>Add Blog Post</button>
        </form>
        <div style={styles.list}>
          {blogs.map(b => (
            <div key={b._id} style={styles.item}>
              <div>
                <h3 style={styles.itemTitle}>{b.title}</h3>
                <p style={styles.itemDesc}>{b.author}</p>
              </div>
...
const styles = {
  ...
  textarea: { width: '100%', padding: '12px', marginBottom: '15px', borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '15px', boxSizing: 'border-box' },
